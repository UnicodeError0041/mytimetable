...
export function loadSavedLessonsFromLocalStorage(): SavedLessons {
	...
	let saveIds = $state(
		browser ? (JSON.parse(localStorage.getItem(SAVE_LESSON_IDS_KEY) ?? '[]') as string[]) : []
	);
	...
	let saves = $state(
		saveIds
			.map((id) =>
				browser
					? JSON.parse(localStorage.getItem(`${SAVE_LESSON_KEY_PREFIX}${id}`) ?? 'null')
					: null
			)
	...
	const createSave = (name?: string, lessons?: LessonData[]) => {
		...
		lessons ??= [];
		name ??= Math.random() < 0.05 ? m.lost_the_game() : m.new_timetable();
	...
	const moveSave = (id: string, idx: number) => {
		if (
			...
			idx < 0 ||
			idx >= saveIds.length ||
			(!areSavesUpToDate() && !ignoreSavesUpToDate)
		) {
			return false;
		}

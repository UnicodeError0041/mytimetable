...
<div class="combobox {classes}">
	<label class="combobox__label label" {...combobox.label}
		>{#if required}<span class="--required">*</span>{/if}
		{label}</label
	>
	<div class="combobox__holder">
		<input
			class="combobox__input text-input text-input--secondary"
			{...combobox.input}
			value={combobox.value === undefined ? '' : optionText(combobox.value)}
		/>
	...
	<div class="combobox__content" {...combobox.content}>
		...
		{:else}
			<p class="combobox__no-results">{m.filter_no_option()}</p>
		{/each}
	</div>
</div>

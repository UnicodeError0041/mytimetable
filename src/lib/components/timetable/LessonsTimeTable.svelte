<script lang="ts">
	...
	import {
		SYMBOL_OPEN_LESSON_EXPORT_MODAL,
		...
		if (tableState === 'default') {
			for (const data of queriedLessons) {
				if (
					...
					!data.lesson.endTime ||
					!data.lesson.day ||
					returned.some((d) => d.data.lessonData.id === data.id)
				) {
					continue;
				}
	...
	const openLessonCopyModal = getContext(SYMBOL_OPEN_LESSON_EXPORT_MODAL) as (
		lessons: LessonData[]
	) => void;

	let differentSemesterWarningDialog: HTMLDialogElement = $state(null!);
	let differentSemesterWarningDialogConfig:
		{ current_semester: string; new_lesson: LessonData } | undefined = $state(undefined);
...
{#snippet element(data: SourcedLessonData)}
	...
	{#if data.source === 'saved'}
		{#snippet savedLessonTooltipContent()}
			...
			<Tooltip content="Óra törlése">
				<button
					...
					onclick={() => {
						if (!currentManager.remove(data.lessonData.id)) {
							openOperationWarningModal();
						}
					}}
				>
		...
		<Lesson
			lesson={data.lessonData.lesson}
			{isEdited}
			classes="lesson--saved --pulse-on-hover {lessonViewOnly} {isLecture ? 'lesson--lecture' : ''}"
	...
	{:else if data.source === 'queried'}
		<Lesson
			lesson={data.lessonData.lesson}
			{isEdited}
			classes="lesson--queried --pulse-on-hover {lessonViewOnly} {isLecture
				? 'lesson--lecture'
				: ''}"
			extraTooltipElement={m.lesson_click_to_add()}
			attrs={{
				onclick: () => saveLesson(data.lessonData),
				role: 'button'
			}}
			triggerType="hover"
		/>
	{:else if data.source === 'alternative'}
		<Lesson
			lesson={data.lessonData.lesson}
			{isEdited}
			classes="lesson--alternative --pulse {lessonViewOnly} {isLecture ? 'lesson--lecture' : ''}"
			triggerType="both"
			extraTooltipElement={m.lesson_drop_here()}
			attrs={{
...
<div class="lesson-timetable">
	...
	<div
		class="lesson-timetable__remove-lesson --pulse
	...
	<LessonDialog
		...
		bind:dialogOpenHandler={editLessonDialogOpenHandler}
		onSave={editEndHandler}
	/>
</div>

<dialog class="dialog" bind:this={differentSemesterWarningDialog}>
	...
	{#if differentSemesterWarningDialogConfig}
		<div class="icon-text --warning">
			...
			<p>
				Ez a kurzus <em
					>{semesterToString(
						differentSemesterWarningDialogConfig.new_lesson.lesson.semester as Semester
					)}</em
				>
				szemeszterhez tartozik, míg az órarend többi órája
				<em>{differentSemesterWarningDialogConfig.current_semester}</em> beli
			</p>
		</div>
		...
		<div class="dialog__buttons">
			...
			>
			<button class="button --pulse-on-hover" onclick={() => differentSemesterWarningDialog.close()}
				>Mégsem</button
			>
		</div>
	{/if}
